...
from .Network_trans import Net_Trans

class ATT_Trans_Deblur_Net(nn.Module, BaseModel):
    def __init__(self, args) -> None:
        super(ATT_Trans_Deblur_Net, self).__init__()
        BaseModel.__init__(self, args)
        self.level = args.level
        if args.net == 'Net_Trans':
            self.net = Net_Trans(level = args.level, style = args.style, mean_shift = args.mean_shift, range_of_image = args.range_of_image)
        else:
        ...
        if self.isTrain:
            ...
            self.train_loss_names += ['l1','l1_1','l1_2','l1_3',
                                      'mse','mse_1','mse_2','mse_3',
                                      'consistency_confidence','consistency']
            self.valid_loss_names += ['l1','l1_1','l1_2','l1_3','consistency_confidence','consistency'
                                      ,'mse','mse_1','mse_2','mse_3','psnr']
            self.meter_init()
        else:
    ...
    def train_step(self):
        ...
        with autocast(enabled=self.args.amp):
            ...
            self.train_loss_consistency_confidence =\
                self.lambda_level3 * self.loss_function_l1(self.attention_3,self.downsample_fn(self.attention_2, (64 ,64 ))) +\
                self.lambda_level2 * self.loss_function_l1(self.attention_2,self.downsample_fn(self.attention_1, (128,128)))
                
            self.train_loss_consistency = \
                self.lambda_level3 * self.loss_function_l1(self.restored_images_3, self.downsample_fn(self.restored_images_2, (64 ,64 ))) +\
                self.lambda_level2 * self.loss_function_l1(self.restored_images_2, self.downsample_fn(self.restored_images_1, (128,128)))
    ...
    def valid_step(self):
        with torch.no_grad():
            with autocast(enabled=self.args.amp):
                ...
                self.valid_loss_l1 = self.valid_loss_l1_1 + self.valid_loss_l1_2 + self.valid_loss_l1_3
                ...
                self.valid_loss_mse = self.valid_loss_mse_1 + self.valid_loss_mse_2 + self.valid_loss_mse_3
